...
class LauncherState(QObject):
    ...
    def __init__(self, parent: QObject | None = None, pinned_queries: list[str] | None = None) -> None:
        ...
        self._pinned_queries: deque[str] = deque(maxlen=5)
        self._indexing_status = IndexingStatus()
        for query in pinned_queries or []:
            self.pin_query(query)
    ...
    def toggle_pinned_query(self, query: str) -> bool:
        ...
        if not normalized:
            return False
        if any(item == normalized for item in self._pinned_queries):
            self._pinned_queries = deque(
                [item for item in self._pinned_queries if item != normalized],
                maxlen=self._pinned_queries.maxlen,
            )
            self.pinned_queries_changed.emit(self.pinned_queries)
            return False
        self.pin_query(normalized)
        return True

    def pin_query(self, query: str) -> None:
        normalized = query.strip()
        if not normalized:
            return
        items = [item for item in self._pinned_queries if item != normalized]
        items.insert(0, normalized)
