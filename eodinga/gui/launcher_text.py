...
def format_empty_state_body(
    ...
    if not query:
        ...
        return (
            ...
            "Recent: "
            f"{recent}.{pinned} "
            "Click a launcher chip or press Alt+Up and Alt+Down to browse recent queries, "
...
def format_shortcut_hint(*, has_results: bool, query: str, result_list_has_focus: bool) -> str:
    if not has_results:
        if query:
...
def format_result_list_accessibility(count: int, current_row: int | None, current_hit: SearchHit | None) -> str:
    if count == 0:
...
__all__ = ["format_empty_state_body", "format_result_list_accessibility", "format_shortcut_hint"]
