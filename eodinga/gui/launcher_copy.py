...
def empty_state_content(
    ...
    if not query:
        ...
        body = (
            ...
            "Alt+1 through Alt+9 to open a top hit, Tab to reach query chips, "
...
def shortcut_hint(*, has_query: bool, has_results: bool, result_list_has_focus: bool) -> str:
    if not has_results:
        if has_query:
            return "Refine with ext:, date:, size:, or content: filters. Alt+Up recalls recent queries."
        return "Type a filename, path, or content term. Tab reaches query chips. Alt+Up recalls recent queries."
