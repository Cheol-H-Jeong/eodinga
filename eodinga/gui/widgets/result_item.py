...
_MARK_OPEN = "<mark style='font-weight:700; background:#FDE68A; color:#111827'>"
...
def _mark_text(text: str) -> str:
    return f"{_MARK_OPEN}{escape(text)}</mark>"
...
def highlight_text(text: str, query: str, *, target: str = "name") -> str:
    ...
    for start, end in spans:
        ...
        parts.append(_mark_text(text[start:end]))
        cursor = end
...
def _highlight_fts_snippet(snippet: str) -> str:
    ...
    while True:
        ...
        parts.append(_mark_text(snippet[start + 1:end]))
        cursor = end + 1
    return "".join(parts)
