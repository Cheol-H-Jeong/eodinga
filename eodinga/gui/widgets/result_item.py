...
_MARK_OPEN = "<mark style='font-weight:700; color:#111827; background:#FDE68A; padding:0 1px; border-radius:3px'>"
_MARK_CLOSE = "</mark>"
...
def highlight_text(text: str, query: str, *, target: str = "name") -> str:
    ...
    for start, end in spans:
        ...
        parts.append(f"{_MARK_OPEN}{escape(text[start:end])}{_MARK_CLOSE}")
        cursor = end
...
def _highlight_fts_snippet(snippet: str) -> str:
    ...
    while True:
        ...
        parts.append(f"{_MARK_OPEN}{escape(snippet[start + 1:end])}{_MARK_CLOSE}")
        cursor = end + 1
    return "".join(parts)
