...
from eodinga.gui.launcher_results import ResultListModel, format_indexing_footer, format_indexing_status
    ...
    def _refresh_empty_state(self) -> None:
        ...
        if not query:
            ...
            self.empty_state.set_content(
                "Type to search",
                f"Recent: {recent_queries} Press Alt+Up to recall recent queries, Tab to move to results, Enter to open the top hit, and Ctrl+Enter to reveal its folder.",
                details,
            )
        else:
            self.empty_state.set_content(
...
class LauncherWindow(LauncherPanel):
    def __init__(
        ...
        always_on_top = self._config.launcher.always_on_top if self._config is not None else True
