...
class LauncherPanel(QWidget):
    ...
    def _refresh_shortcut_hint(self) -> None:
        ...
        if not has_results:
            if self.query_field.text().strip():
                hint = "Refine with ext:, date:, size:, or content: filters. Alt+P pins the current query. Alt+Up recalls recent queries."
            else:
                hint = "Type a filename, path, or content term. Alt+P pins the current query. Alt+Up recalls recent queries."
    ...
    def _handle_query_field_keypress(self, event: QKeyEvent) -> bool:
        if event.modifiers() & Qt.KeyboardModifier.AltModifier and event.key() == Qt.Key.Key_P:
