...
class LauncherWindow(LauncherPanel):
    def __init__(
        ...
        self.apply_always_on_top(config.launcher.always_on_top if config is not None else False)
        width = config.launcher.window_width if config is not None else 640
        height = config.launcher.window_height if config is not None else 480
