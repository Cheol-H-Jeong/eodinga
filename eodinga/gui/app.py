...
class EodingaWindow(QMainWindow):
    def __init__(
        ...
        self.settings_tab.set_pinned_queries(resolved_config.launcher.pinned_queries)
        self.settings_tab.set_recent_queries(self.launcher_state.recent_queries)
        ...
        self.settings_tab.pinned_queries_changed.connect(self._change_pinned_queries)
        self.settings_tab.clear_recent_queries_requested.connect(self.launcher_state.clear_recent_queries)
        self.launcher_state.recent_queries_changed.connect(self.settings_tab.set_recent_queries)
    ...
    def _change_pinned_queries(self, queries: list[str]) -> None:
        self.launcher_state.set_pinned_queries(queries)
        self._config.launcher = self._config.launcher.model_copy(update={"pinned_queries": queries})
        self._config.save(self._config_path)
        self.settings_tab.set_pinned_queries(queries)
