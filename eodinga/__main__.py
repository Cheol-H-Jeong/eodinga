...
def _cmd_stats(args: argparse.Namespace) -> int:
    ...
    histograms = metrics_state["histograms"]
    snapshot = StatsSnapshot(
        ...
        queries_served=int(counters.get("queries_served", 0)),
        queries_zero_results=int(counters.get("queries_zero_results", 0)),
        queries_truncated=int(counters.get("queries_truncated", 0)),
        parser_errors=int(counters.get("parser_errors", 0)),
        watcher_events=int(counters.get("watcher_events", 0)),
        watcher_flushes=int(counters.get("watcher_flushes", 0)),
        watcher_events_flushed=int(counters.get("watcher_events_flushed", 0)),
        watcher_queue_full=int(counters.get("watcher_queue_full", 0)),
        watcher_enqueue_aborted=int(counters.get("watcher_enqueue_aborted", 0)),
        watcher_observers_started=int(counters.get("watcher_observers_started", 0)),
        watcher_observers_stopped=int(counters.get("watcher_observers_stopped", 0)),
        index_rebuilds_completed=int(counters.get("index_rebuilds_completed", 0)),
        ...
        crashes_reported=int(counters.get("crashes_reported", 0)),
        crash_logs_written=int(counters.get("crash_logs_written", 0)),
        crash_log_write_failures=int(counters.get("crash_log_write_failures", 0)),
        crash_handlers_installed=int(counters.get("crash_handlers_installed", 0)),
        logging_configurations=int(counters.get("logging_configurations", 0)),
        log_sinks_stderr_configured=int(counters.get("log_sinks.stderr.configured", 0)),
        log_sinks_file_configured=int(counters.get("log_sinks.file.configured", 0)),
        log_sinks_file_disabled=int(counters.get("log_sinks.file.disabled", 0)),
        query_latency_histogram=dict(histograms.get("query_latency_ms", {})),
        query_result_count_histogram=dict(histograms.get("query_result_count", {})),
        command_latency_histogram=dict(histograms.get("command_latency_ms", {})),
        watch_flush_batch_histogram=dict(histograms.get("watch_flush_batch_size", {})),
        watch_event_lag_histogram=dict(histograms.get("watch_event_lag_ms", {})),
        watcher_queue_backpressure_histogram=dict(
            histograms.get("watcher_queue_backpressure_ms", {})
        ),
        index_rebuild_latency_histogram=dict(histograms.get("index_rebuild_latency_ms", {})),
        index_batch_size_histogram=dict(histograms.get("index_batch_size", {})),
        ...
        recent_snapshots=[dict(entry) for entry in metrics_state["recent_snapshots"]],
        roots=list(index_snapshot.roots) or [root.path for root in config.roots],
        db_path=db_path,
        log_path=resolve_log_path(),
        metrics_path=resolve_metrics_path(),
        metrics_persistence_enabled=metrics_state["persistence_enabled"],
