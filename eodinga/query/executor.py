...
def _compiled_regex(pattern: str, flags: str, default_case_sensitive: bool) -> re.Pattern[str]:
    compiled_flags = _make_flags(flags)
    if not default_case_sensitive and "i" not in flags.lower():
...
def _should_scan_path_candidates(branch: CompiledBranch, fts_ids: list[int]) -> bool:
    ...
    return any(
        term.kind == "phrase" or any(ord(char) > 127 for char in term.value) for term in positive_terms
    )
