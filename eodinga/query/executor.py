...
def _fetch_path_candidates_python_scan(
    ...
    matched: list[tuple[int, str, str, int, FileRecord]] = []
    for record in records.values():
        normalized_name = _normalize_search_text(record.name, case_sensitive=branch.case_sensitive)
        normalized_path = _normalize_search_text(str(record.path), case_sensitive=branch.case_sensitive)
        if not all(needle in normalized_name or needle in normalized_path for needle in needles):
            continue
        matched.append(
            (
                0 if any(normalized_name.startswith(needle) for needle in needles) else 1,
                normalized_name,
                str(record.path),
    ...
    ordered = [record for _, _, _, _, record in sorted(matched)[:limit]]
...
def _scan_auto_content_candidates(
    ...
    while len(matched) < target:
        ...
        for file_id, record in batch.items():
            ...
            if not all(needle in normalized_content for needle in needles):
                continue
            matched[file_id] = record
            if len(matched) >= target:
                break
...
def _prefix_hits(records: Mapping[int, FileRecord], branch: CompiledBranch) -> list[int]:
    needles = _normalized_needles(
        (term.value for term in branch.path_terms if not term.negated),
        case_sensitive=branch.case_sensitive,
    )
    if not needles:
        return []
...
def _derive_name_path_hits(
    ...
    for record in records.values():
        ...
        if any(needle in target_name for needle in needles):
            name_hits.append(record.id)
        if any(needle in target_path for needle in needles):
