...
class _Parser:
    ...
    def _parse_regex(self) -> RegexNode:
        ...
        delimiters = self._regex_delimiters(remaining)
        if len(delimiters) < 2 or remaining.endswith("\\"):
            raise QuerySyntaxError("unterminated regex", start)
        closing_index = delimiters[1]
        pattern_start = self.index + 1
        pattern = self.source[pattern_start : self.index + closing_index]
