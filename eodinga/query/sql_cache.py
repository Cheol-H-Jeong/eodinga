...
def record_batch_sql(where_sql: str) -> str:
...
@lru_cache(maxsize=128)
def content_candidates_sql(content_match_sql: str, where_sql: str) -> str:
    sql = """
...
@lru_cache(maxsize=128)
def auto_content_candidates_sql(where_sql: str) -> str:
    sql = """
        SELECT files.*, snippet(content_fts, 2, '[', ']', '...', 12) AS snippet
        FROM content_fts
        JOIN content_map ON content_map.fts_rowid = content_fts.rowid
...
@lru_cache(maxsize=128)
def content_backfill_sql(where_sql: str) -> str:
    sql = """
        SELECT files.*
        FROM files
...
@lru_cache(maxsize=128)
def fetch_content_texts_sql(chunk_size: int) -> str:
