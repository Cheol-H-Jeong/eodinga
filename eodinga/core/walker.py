...
from eodinga.core.fs import resolve_safe, scandir_with_stat_safe, stat_follow_safe, stat_safe
from eodinga.core.rules import should_index

BATCH_SIZE = 8192


def _to_record(root_id: int, path: Path, stat_result: os_stat_result) -> FileRecord:
...
def _should_descend(path: Path, root: Path, stat_result: os_stat_result) -> bool:
...
def walk_batched(root: Path, rules: PathRules, root_id: int = 0) -> Iterator[list[FileRecord]]:
    queue: deque[tuple[Path, os_stat_result | None]] = deque([(root, None)])
    visited_dirs: set[tuple[int, int]] = set()
    visited_resolved_dirs: set[Path] = set()
    batch: list[FileRecord] = []
    while queue:
        current, discovered_stat = queue.popleft()
        try:
            stat_result = discovered_stat if discovered_stat is not None else stat_safe(current)
        except OSError:
            continue
        ...
        try:
            children = scandir_with_stat_safe(current)
        except OSError:
            continue
        queue.extend((child.path, child.stat_result) for child in children)
    if batch:
        yield batch
