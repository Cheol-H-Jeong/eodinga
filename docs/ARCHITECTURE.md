...
Use this table when a user-visible symptom is vague but you still want the first investigation step to land on one subsystem boundary:

| Symptom | Start at this layer | First command or artifact | Why this layer goes first |
| --- | --- | --- | --- |
| stale search results | storage + runtime path ownership | `eodinga stats --json` | stale output is often a wrong database path or unfinished rebuild before it is a ranker bug |
| launcher differs from CLI | config + launcher surface | `eodinga doctor` and config path review | the query engine is shared, so surface drift usually means config or hotkey/runtime wiring first |
| filesystem changes do not appear | watcher pipeline | `eodinga watch` plus watcher counters in `eodinga stats --json` | the watch-to-query path is where debounce, queueing, and incremental writes meet |
| startup reports recovery work repeatedly | storage recovery path | index directory sidecars plus `eodinga doctor` | `.next`, `.recover`, and WAL sidecars identify whether startup is resuming or stuck |
| packaging docs disagree with artifacts | packaging surfaces | `packaging/dist/*-audit.json` | release docs are claims about generated payloads, so the staged audit manifests are the source of truth |

The aim is to keep incident response architecture-shaped: identify the boundary that owns the symptom before widening the search to unrelated modules.

## Command-To-Subsystem Map

This is the shortest bridge from an operator command to the code that should explain its output:

| Command or artifact | Primary modules | What it proves |
| --- | --- | --- |
| `eodinga doctor` | `eodinga.__main__`, `eodinga.core.fs`, launcher/backend detection helpers | environment shape, writable runtime paths, and launch-surface prerequisites |
| `eodinga stats --json` | `eodinga.__main__`, `eodinga.index.storage`, observability helpers | active database path, runtime counters, and watcher/index state |
| `eodinga search ... --json` | `eodinga.query.dsl`, `eodinga.query.compiler`, `eodinga.query.executor`, `eodinga.query.ranker` | the shared query engine behavior independent of GUI rendering |
| `eodinga watch` | `eodinga.core.watcher`, `eodinga.index.writer` | incremental update flow from filesystem events into committed index rows |
| `python packaging/build.py --target ...-dry-run` | `packaging/build.py`, `packaging/windows/*`, `packaging/linux/*` | rendered package payload claims and staged review artifacts under `packaging/dist/` |

When a command and its owning subsystem disagree, trust the artifact generated by that command first and only then move outward to the adjacent layers.
