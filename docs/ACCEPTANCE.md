...
- `linux-appimage-dry-run` and `linux-deb-dry-run` must validate the Linux packaging recipes and staging manifests without emitting final artifacts.
- `yamllint` validates the release workflow YAML shipped in both `.github/workflows/release-windows.yml` and `.github/workflows/release-linux.yml`.
...
- feature highlights for the local-only search stack
- the default launcher hotkey and keyboard actions
- the DSL entry points and a link to `docs/DSL.md`
- links to `docs/ARCHITECTURE.md`, `docs/CONTRIBUTING.md`, `docs/PERFORMANCE.md`, and `docs/RELEASE.md`
