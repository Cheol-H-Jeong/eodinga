...
- `tests/perf/test_query_latency.py::test_unicode_name_query_latency`: Unicode path-query latency against a 50k-file index with Hangul-heavy filenames.
...
- `EODINGA_PERF_QUERY_FILE_COUNT`, `EODINGA_PERF_QUERY_COUNT`, `EODINGA_PERF_QUERY_P95_MS`, `EODINGA_PERF_UNICODE_QUERY_P95_MS`
...
Measured on 2026-04-23 in this repository’s Linux dev environment with `.venv` dependencies installed after the 0.1.78 walker and Unicode-query batching round:

| Benchmark | Dataset | Result |
| --- | --- | --- |
| Cold start | 20,201 indexed entries | 5,632 files/sec |
| Rebuild cold start | 20,201 indexed entries via `rebuild_index()` | 5,956 files/sec |
| Bulk upsert | 50k synthetic records | 50,589 records/sec |
| Name query latency | 2,000 queries / 50k files | p50 0.06 ms, p95 0.07 ms, p99 0.07 ms |
| Unicode name query latency | 2,000 queries / 50k files | p50 8.72 ms, p95 11.29 ms, p99 12.03 ms |
| Content query latency | 500 queries / 5k docs | p50 0.66 ms, p95 0.71 ms, p99 0.76 ms |
| Watch latency | 25 created files | p99 0.131 s |

These numbers are informational for v0.1, not release-blocking. The thresholds in `tests/perf/*` are set to catch clear regressions on a normal developer workstation rather than to enforce the SPEC’s reference-box targets. This round trims two hotspots: walker traversal now reuses `os.scandir()` metadata for discovered children instead of immediately re-`lstat()`ing them, and Unicode path queries stop scanning once the prefix-priority result window is full instead of materializing a 100k-row fallback candidate set.
...
- `tests/perf/test_query_latency.py` isolates both ASCII and Unicode name/path lookup cost without parser noise.
