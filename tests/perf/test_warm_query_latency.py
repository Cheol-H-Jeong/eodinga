...
FILE_COUNT = perf_int_env("EODINGA_PERF_WARM_QUERY_FILE_COUNT", 40_000)
QUERY_COUNT = perf_int_env("EODINGA_PERF_WARM_QUERY_COUNT", 1_000)
P95_LIMIT_MS = perf_float_env("EODINGA_PERF_WARM_QUERY_P95_MS", 12.0)


def test_warm_query_latency(tmp_path: Path) -> None:
    root = tmp_path / "tree"
    root.mkdir()
    conn = open_perf_db(tmp_path / "warm-query-latency.db")
    try:
        ...
        for index in range(FILE_COUNT):
            ...
            name = f"report-{index:05d}.txt" if index % 3 == 0 else f"note-{index:05d}.txt"
            records.append(make_file_record(group / name, size=index))
        writer.bulk_upsert(records)

        query = "report-00030"
        warm_result = search(conn, query, limit=10)
        assert warm_result.hits
