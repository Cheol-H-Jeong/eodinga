...
FILE_COUNT = perf_int_env("EODINGA_PERF_WARM_QUERY_FILE_COUNT", 50_000)
QUERY_COUNT = perf_int_env("EODINGA_PERF_WARM_QUERY_COUNT", 2_000)
P95_LIMIT_MS = perf_float_env("EODINGA_PERF_WARM_QUERY_P95_MS", 20.0)


def test_name_query_warm_latency(tmp_path: Path) -> None:
    root = tmp_path / "tree"
    root.mkdir()
    ...
    try:
        ...
        assert search(conn, warmup_query, limit=10).hits
