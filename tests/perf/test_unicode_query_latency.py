...
QUERY_COUNT = perf_int_env("EODINGA_PERF_UNICODE_QUERY_COUNT", 500)
...
def test_unicode_name_query_latency(tmp_path: Path) -> None:
    ...
    try:
        ...
        for index in range(FILE_COUNT):
            group = root / f"문서-{index % 128:03d}"
            name = f"회의록-{index:05d}.txt" if index % 5 == 0 else f"메모-{index:05d}.txt"
            records.append(make_file_record(group / name, size=index))
        writer.bulk_upsert(records)

        queries = [f"회의록-{random.randrange(FILE_COUNT // 5) * 5:05d}" for _ in range(QUERY_COUNT)]
