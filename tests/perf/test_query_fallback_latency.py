...
FILE_COUNT = perf_int_env("EODINGA_PERF_FALLBACK_QUERY_FILE_COUNT", 2_000)
QUERY_COUNT = perf_int_env("EODINGA_PERF_FALLBACK_QUERY_COUNT", 100)
P95_LIMIT_MS = perf_float_env("EODINGA_PERF_FALLBACK_QUERY_P95_MS", 80.0)


def test_regex_fallback_query_latency(tmp_path: Path) -> None:
    root = tmp_path / "tree"
    root.mkdir()
    ...
    try:
        ...
        for index in range(FILE_COUNT):
            ...
            name = f"report-{index:05d}.txt"
            if index % 97 == 0:
                name = f"needle-{index:05d}.txt"
        ...
        for _ in range(QUERY_COUNT):
            ...
            result = search(conn, "/needle-[0-9]+/", limit=10)
