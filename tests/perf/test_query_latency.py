...
REGEX_P95_LIMIT_MS = perf_float_env("EODINGA_PERF_REGEX_QUERY_P95_MS", 45.0)
...
def test_regex_query_latency(tmp_path: Path) -> None:
    ...
    try:
        ...
        queries = [rf"/report-{random.randrange(FILE_COUNT // 5) * 5:05d}/" for _ in range(QUERY_COUNT)]
        latencies_ms: list[float] = []
        for query in queries:
            started = perf_counter()
            result = search(conn, query, limit=10)
        ...
        print(
            ...
            f"files={FILE_COUNT} count={QUERY_COUNT} "
            f"p50={p50:.2f}ms p95={p95:.2f}ms p99={p99:.2f}ms "
            f"limit_p95={REGEX_P95_LIMIT_MS:.2f}ms"
        )
        assert p95 <= REGEX_P95_LIMIT_MS
    finally:
        conn.close()
