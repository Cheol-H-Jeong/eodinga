...
MIN_FILES_PER_SECOND = perf_float_env("EODINGA_PERF_WALK_MIN_FPS", 30_000.0)
...
def test_walk_batched_throughput(tmp_path: Path) -> None:
    ...
    count = sum(len(batch) for batch in walk_batched(root, make_walk_rules(root), root_id=1))
    elapsed = perf_counter() - started
    throughput = count / elapsed

    print(
        "walk_throughput "
        f"files={count} elapsed={elapsed:.3f}s throughput={throughput:.0f} files/s "
        f"min_fps={MIN_FILES_PER_SECOND:.0f}"
    )
    assert count == FILE_COUNT + 257
