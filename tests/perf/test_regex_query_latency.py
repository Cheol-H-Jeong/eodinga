...
FILE_COUNT = perf_int_env("EODINGA_PERF_REGEX_QUERY_FILE_COUNT", 50_000)
QUERY_COUNT = perf_int_env("EODINGA_PERF_REGEX_QUERY_COUNT", 500)
P95_LIMIT_MS = perf_float_env("EODINGA_PERF_REGEX_QUERY_P95_MS", 80.0)


def test_regex_query_latency(tmp_path: Path) -> None:
    root = tmp_path / "regex-tree"
    root.mkdir()
    ...
    try:
        ...
        for index in range(FILE_COUNT):
            ...
            name = f"report-{index:05d}.txt" if index % 7 == 0 else f"note-{index:05d}.txt"
            records.append(make_file_record(group / name, size=index))
        writer.bulk_upsert(records)

        queries = [
            rf"regex:/report-{random.randrange(FILE_COUNT // 7) * 7:05d}/"
