...
def test_trimmed_multi_root_reopen_live_delete_stays_in_surviving_scope(tmp_path: Path) -> None:
    root_a = tmp_path / "alpha-root"
    root_b = tmp_path / "beta-root"
    db_path = tmp_path / "database" / "index.db"
    root_a.mkdir()
    root_b.mkdir()
    target = root_a / "alpha-delete.txt"
    removed_root_file = root_b / "beta-pruned.txt"
    target.write_text("trimmed root delete target\n", encoding="utf-8")
    removed_root_file.write_text("trimmed root pruned\n", encoding="utf-8")
    ...
    try:
        ...
        initial_hits = {hit.file.path for hit in search(reopened, "delete target", limit=10).hits}
        ...
        global_hits = {hit.file.path for hit in search(reopened, "delete target", limit=10).hits}
        alpha_hits = {
            hit.file.path for hit in search(reopened, "delete target", limit=10, root=root_a).hits
        }
        beta_hits = {
            hit.file.path for hit in search(reopened, "delete target", limit=10, root=root_b).hits
        }
        pruned_hits = {hit.file.path for hit in search(reopened, "pruned", limit=10).hits}
        stored_roots = {
            Path(row[0]) for row in reopened.execute("SELECT path FROM roots ORDER BY id").fetchall()
