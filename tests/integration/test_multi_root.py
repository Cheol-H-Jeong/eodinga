...
def test_multi_root_rebuild_reopen_preserves_reduced_root_scope(tmp_path: Path) -> None:
    root_a = tmp_path / "alpha-root"
    root_b = tmp_path / "beta-root"
    db_path = tmp_path / "database" / "index.db"
    ...
    alpha = root_a / "alpha-keep.txt"
    beta = root_b / "beta-drop.txt"
    alpha.write_text("shared reopen survivor\n", encoding="utf-8")
    beta.write_text("shared reopen survivor\n", encoding="utf-8")
    ...
    try:
        hits = {hit.file.path for hit in search(reopened, "shared reopen survivor", limit=10).hits}
        alpha_hits = {
            hit.file.path for hit in search(reopened, "shared reopen survivor", limit=10, root=root_a).hits
        }
        beta_hits = {
            hit.file.path for hit in search(reopened, "shared reopen survivor", limit=10, root=root_b).hits
        }
        stored_roots = {
