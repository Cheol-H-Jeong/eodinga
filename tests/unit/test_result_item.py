...
def test_highlight_text_supports_path_and_extension_filters_by_target() -> None:
    ...
    assert "/tmp/<mark style='font-weight:700; padding:0 1px'>reports</mark>/quarterly.pdf" in rendered_path
    assert "<mark style='font-weight:700; padding:0 1px'>pdf</mark>" in rendered_ext
...
def test_format_hit_html_shows_parent_path_line_for_path_filters() -> None:
    ...
    assert "/workspace/<mark style='font-weight:700; padding:0 1px'>reports</mark>" in rendered
...
def test_format_hit_html_prefers_precomputed_highlighted_path() -> None:
    rendered = format_hit_html(
        SearchHit(
            ...
            name="release-notes.txt",
            ext="txt",
            highlighted_path="/workspace/<mark style='font-weight:700; padding:0 1px'>reports</mark>",
        ),
        "path:ignored",
    )
