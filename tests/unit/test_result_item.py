...
def test_highlight_text_marks_all_case_insensitive_matches() -> None:
    ...
    assert "<strong>Report</strong>" in rendered
    assert "<strong>report</strong>" in rendered
    assert "<strong>REPORT</strong>" in rendered
    assert "background:#CCFBF1" in rendered
...
def test_highlight_text_ignores_dsl_filters_and_marks_free_text_terms() -> None:
    ...
    assert "<strong>release</strong>" in rendered
...
def test_highlight_text_marks_quoted_phrases() -> None:
    ...
    assert "<strong>release notes</strong>" in rendered
...
def test_highlight_text_marks_escaped_quoted_phrases() -> None:
    ...
    assert "<strong>release &quot;candidate&quot;</strong>" in rendered
...
def test_highlight_text_ignores_negated_terms() -> None:
    ...
    assert "<strong>release</strong>" in rendered
    assert "<strong>draft</strong>" not in rendered
...
def test_highlight_text_supports_path_and_extension_filters_by_target() -> None:
    ...
    assert "/tmp/<mark style='background:#CCFBF1; color:#0F172A; padding:0 1px;'><strong>reports</strong></mark>/quarterly.pdf" in rendered_path
    assert "<strong>pdf</strong>" in rendered_ext
...
def test_highlight_text_supports_regex_and_content_filters() -> None:
    ...
    assert "<strong>release-notes</strong></mark>.txt" in rendered_name
    assert "<strong>release notes</strong>" in rendered_snippet
...
def test_highlight_text_respects_case_true_for_literals() -> None:
    ...
    assert "<strong>Report</strong>" in rendered
    assert "<strong>report</strong>" not in rendered
...
def test_format_hit_html_renders_extension_badge() -> None:
    ...
    assert "<strong>report</strong></mark>.pdf" in rendered
...
def test_format_hit_html_shows_parent_path_line_for_path_filters() -> None:
    ...
    assert "<strong>reports</strong>" in rendered
