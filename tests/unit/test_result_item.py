...
def test_format_preview_html_highlights_name_path_and_snippet() -> None:
    title, path_text, snippet = format_preview_html(
        SearchHit(
            path=Path("/workspace/reports/release-notes.txt"),
            parent_path=Path("/workspace/reports"),
            name="release-notes.txt",
        ...
        'release path:reports content:"release notes"',
    )

    assert "release</mark>-notes.txt" in title
    assert "/workspace/<mark style='font-weight:700; background-color:#FDE68A; color:#111827'>reports</mark>" in path_text
    assert "<mark style='font-weight:700; background-color:#FDE68A; color:#111827'>release notes</mark>" in snippet
