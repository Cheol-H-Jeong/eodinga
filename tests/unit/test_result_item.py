...
def test_highlight_text_marks_all_case_insensitive_matches() -> None:
    ...
    assert "background:#CFFAFE" in rendered
...
def test_highlight_text_supports_path_and_extension_filters_by_target() -> None:
    ...
    assert "/tmp/<mark style='font-weight:700; background:#CFFAFE; color:inherit; padding:0 1px; border-radius:3px'>reports</mark>/quarterly.pdf" in rendered_path
...
def test_format_hit_html_shows_parent_path_line_for_path_filters() -> None:
    ...
    assert "/workspace/<mark style='font-weight:700; background:#CFFAFE; color:inherit; padding:0 1px; border-radius:3px'>reports</mark>" in rendered
...
def test_format_hit_html_prefers_precomputed_highlighted_path() -> None:
    rendered = format_hit_html(
        SearchHit(
            ...
            name="release-notes.txt",
            ext="txt",
            highlighted_path="/workspace/<mark style='font-weight:700; background:#CFFAFE; color:inherit; padding:0 1px; border-radius:3px'>reports</mark>",
        ),
        "path:ignored",
    )
