...
def test_rebuild_index_interrupt_keeps_existing_target_database(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "root"
    root.mkdir()
    ...
    fake_interrupts = FakeInterruptState()

    @contextmanager
    def fake_defer_termination_signals() -> Iterator[FakeInterruptState]:
        yield fake_interrupts
    ...
    def interrupted_walk_batched(root_path: Path, rules, root_id: int = 0):
        for batch in original_walk_batched(root_path, rules, root_id=root_id):
            fake_interrupts.requested = True
            yield batch

    monkeypatch.setattr(build_module, "_defer_termination_signals", fake_defer_termination_signals)
    monkeypatch.setattr(build_module, "walk_batched", interrupted_walk_batched)

    with pytest.raises(KeyboardInterrupt, match="index rebuild interrupted by SIGTERM"):
        rebuild_index(db_path, [RootConfig(path=root)])
