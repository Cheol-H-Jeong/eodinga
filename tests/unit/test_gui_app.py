...
def test_app_accessible_names_cover_main_interactive_widgets(qapp) -> None:
    ...
    assert window.roots_tab.accessibleDescription() == "Manage which filesystem roots are indexed."
    ...
    assert window.roots_tab.add_root_button.accessibleDescription() == "Add a filesystem root to the index."
    assert window.roots_tab.remove_root_button.accessibleName() == "Remove selected root"
    assert window.roots_tab.remove_root_button.accessibleDescription() == "Remove the currently selected filesystem root."
    assert window.index_tab.accessibleName() == "Index tab"
    assert window.index_tab.accessibleDescription() == "Observe index health and trigger a rebuild."
    ...
    assert window.index_tab.rebuild_button.accessibleDescription() == "Rebuild the full search index from the configured roots."
    assert window.search_tab.accessibleName() == "Search tab"
    assert window.search_tab.accessibleDescription() == "Search the index with the embedded launcher controls."
    assert window.settings_tab.accessibleName() == "Settings tab"
    assert window.settings_tab.accessibleDescription() == "Configure launcher behavior, hotkeys, and theme settings."
    ...
    assert window.settings_tab.system_theme_checkbox.accessibleDescription() == "Follow the operating system theme when supported."
    assert window.settings_tab.frameless_checkbox.accessibleName() == "Use frameless launcher window"
    assert window.settings_tab.frameless_checkbox.accessibleDescription() == "Remove the standard window frame from the launcher."
    assert window.settings_tab.always_on_top_checkbox.accessibleName() == "Keep launcher always on top"
    assert window.settings_tab.always_on_top_checkbox.accessibleDescription() == "Keep the launcher above other windows while visible."
    assert window.settings_tab.hotkey_label.accessibleName() == "Current launcher hotkey"
    assert window.settings_tab.hotkey_label.accessibleDescription() == "Shows the currently registered launcher hotkey."
    assert window.settings_tab.remap_hotkey_button.accessibleName() == "Remap hotkey"
    assert window.settings_tab.remap_hotkey_button.accessibleDescription() == "Choose a new global shortcut for opening the launcher."
    assert window.about_tab.accessibleName() == "About tab"
    assert window.about_tab.accessibleDescription() == "Show the app version and a short product summary."
