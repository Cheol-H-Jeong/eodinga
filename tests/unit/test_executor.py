...
def test_execute_iso_week_literals_match_full_calendar_week(tmp_db: sqlite3.Connection) -> None:
    _insert_file(
        tmp_db,
        1,
        "/workspace/in-week.txt",
        512,
        int(datetime(2026, 4, 21, 12, 0, tzinfo=UTC).timestamp()),
        "txt",
        body_text="inside 2026-W17",
    )
    _insert_file(
        tmp_db,
        2,
        "/workspace/out-week.txt",
        512,
        int(datetime(2026, 4, 27, 12, 0, tzinfo=UTC).timestamp()),
        "txt",
        body_text="outside 2026-W17",
    )
    tmp_db.commit()

    hits = [hit.file.name for hit in search(tmp_db, "date:2026-W17", limit=10).hits]

    assert hits == ["in-week.txt"]
