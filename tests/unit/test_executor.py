...
def test_relative_date_queries_do_not_reuse_stale_compile_cache(
    tmp_db: sqlite3.Connection, monkeypatch: pytest.MonkeyPatch
) -> None:
    seoul = ZoneInfo("Asia/Seoul")
    first_now = datetime(2026, 4, 23, 0, 30, tzinfo=seoul)
    second_now = datetime(2026, 4, 24, 0, 30, tzinfo=seoul)
    first_hit = int(datetime(2026, 4, 23, 12, 0, tzinfo=seoul).timestamp())
    second_hit = int(datetime(2026, 4, 24, 12, 0, tzinfo=seoul).timestamp())
    ...
    _insert_file(tmp_db, 1, "/workspace/day-one.txt", 512, first_hit, "txt", body_text="day one")
    _insert_file(tmp_db, 2, "/workspace/day-two.txt", 512, second_hit, "txt", body_text="day two")
    tmp_db.commit()

    first_hits = [hit.file.name for hit in search(tmp_db, "date:today", limit=10).hits]
    _FrozenDateTime.current = second_now
