...
def test_execute_negated_and_group_query_applies_demorgan_truth_table(
    tmp_db: sqlite3.Connection,
) -> None:
    now = 1_713_528_000
    _insert_file(tmp_db, 1, "/workspace/alpha-beta.txt", 1024, now, "txt", body_text="alpha beta")
    _insert_file(tmp_db, 2, "/workspace/alpha-only.txt", 1024, now - 60, "txt", body_text="alpha")
    _insert_file(tmp_db, 3, "/workspace/beta-only.txt", 1024, now - 120, "txt", body_text="beta")
...
def test_search_root_scope_matches_windows_root_record_across_drive_letter_case(
    tmp_db: sqlite3.Connection,
) -> None:
    now = 1_713_528_000
