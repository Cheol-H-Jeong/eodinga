...
def test_executor_caches_compiled_regexes() -> None:
    executor_module._compiled_regex.cache_clear()

    executor_module._compiled_regex("alpha.*beta", "i", False)
    executor_module._compiled_regex("alpha.*beta", "i", False)
    executor_module._compiled_regex("alpha.*beta", "", True)
    executor_module._compiled_regex("alpha.*beta", "", True)

    cache_info = executor_module._compiled_regex.cache_info()
    assert cache_info.hits >= 2
    assert cache_info.currsize == 2
