...
def test_execute_open_ended_size_range_queries(tmp_db: sqlite3.Connection) -> None:
    now = 1_713_528_000
    _insert_file(tmp_db, 1, "/workspace/empty.txt", 0, now, "txt", body_text="")
    _insert_file(tmp_db, 2, "/workspace/small.txt", 100, now - 60, "txt", body_text="small")
    _insert_file(tmp_db, 3, "/workspace/medium.txt", 500 * 1024, now - 120, "txt", body_text="medium")
    _insert_file(tmp_db, 4, "/workspace/large.txt", 500 * 1024 + 1, now - 180, "txt", body_text="large")
    tmp_db.commit()
    ...
    spaced_hits = [hit.file.name for hit in search(tmp_db, "size:.. 500K", limit=10).hits]
    negated_hits = [hit.file.name for hit in search(tmp_db, "-size:..500K", limit=10).hits]

    assert smaller_hits == ["empty.txt", "medium.txt", "small.txt"]
    assert larger_hits == ["large.txt", "medium.txt", "small.txt"]
    assert spaced_hits == smaller_hits
