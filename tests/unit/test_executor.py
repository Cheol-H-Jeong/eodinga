...
def test_execute_open_ended_size_range_queries(tmp_db: sqlite3.Connection) -> None:
    ...
    _insert_file(tmp_db, 3, "/workspace/medium.txt", 400 * 1024, now - 120, "txt", body_text="medium")
    _insert_file(tmp_db, 4, "/workspace/large.txt", 2 * 1024 * 1024, now - 180, "txt", body_text="large")
    tmp_db.commit()
    ...
    negated_hits = [hit.file.name for hit in search(tmp_db, "-size:..100", limit=10).hits]

    assert smaller_hits == ["medium.txt", "small.txt", "tiny.txt"]
    assert larger_hits == ["large.txt", "medium.txt", "small.txt"]
    assert negated_hits == ["large.txt", "medium.txt"]
