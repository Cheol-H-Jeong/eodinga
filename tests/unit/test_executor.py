...
def test_execute_path_filter_with_escaped_slash_regex(tmp_db: sqlite3.Connection) -> None:
    now = 1_713_528_000
    _insert_file(tmp_db, 1, "/workspace/foo/bar.txt", 512, now, "txt", body_text="alpha")
    _insert_file(tmp_db, 2, "/workspace/foo-baz.txt", 512, now - 60, "txt", body_text="beta")
    tmp_db.commit()

    hits = [
        hit.file.path.as_posix() for hit in search(tmp_db, r"path:/foo\/bar/i", limit=5).hits
    ]

    assert hits == ["/workspace/foo/bar.txt"]
...
def test_execute_ascii_middle_token_filename_query(tmp_db: sqlite3.Connection) -> None:
    now = 1_713_528_000
    _insert_file(tmp_db, 1, "/workspace/docs/alpha-gamma.txt", 1024, now, "txt", body_text="alpha")
    _insert_file(tmp_db, 2, "/workspace/docs/gamma.txt", 1024, now - 60, "txt", body_text="gamma")
    _insert_file(tmp_db, 3, "/workspace/docs/beta.txt", 1024, now - 120, "txt", body_text="beta")
    tmp_db.commit()

    hits = [hit.file.name for hit in search(tmp_db, "gamma", limit=10).hits]

    assert hits == ["gamma.txt", "alpha-gamma.txt"]
...
def test_search_root_scope_matches_windows_root_exactly_across_drive_letter_case(
    tmp_db: sqlite3.Connection,
) -> None:
    now = 1_713_528_000
    _insert_file(tmp_db, 1, r"C:\workspace\reports", 0, now, "", is_dir=True)
