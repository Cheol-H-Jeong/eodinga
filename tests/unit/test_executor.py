...
def test_execute_nested_negated_group_query_applies_demorgan_truth_table(
    tmp_db: sqlite3.Connection,
) -> None:
    now = 1_713_528_000
    _insert_file(tmp_db, 1, "/workspace/alpha-gamma.txt", 1024, now, "txt", body_text="alpha gamma")
    _insert_file(tmp_db, 2, "/workspace/beta-gamma.txt", 1024, now - 60, "txt", body_text="beta gamma")
    _insert_file(tmp_db, 3, "/workspace/alpha-only.txt", 1024, now - 120, "txt", body_text="alpha")
    _insert_file(tmp_db, 4, "/workspace/gamma-only.txt", 1024, now - 180, "txt", body_text="gamma")
    _insert_file(tmp_db, 5, "/workspace/neutral.txt", 1024, now - 240, "txt", body_text="neutral")
    tmp_db.commit()
