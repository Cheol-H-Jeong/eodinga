...
def test_launcher_preview_follows_selected_result(qapp) -> None:
    def search_fn(query: str, limit: int) -> QueryResult:
        return QueryResult(
            items=[
                SearchHit(
    ...
    assert launcher.preview_pane.snippet_label.text() == "Alpha preview line"

    QTest.keyClick(launcher.result_list, Qt.Key.Key_Down)

    assert launcher.preview_pane.title_label.text() == "beta.md"
