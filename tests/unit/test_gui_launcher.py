...
def test_launcher_preview_returns_to_selected_result_after_pointer_leaves_results(qapp) -> None:
    def search_fn(query: str, limit: int) -> QueryResult:
        return QueryResult(
            items=[
                SearchHit(path=Path("/tmp/alpha.txt"), parent_path=Path("/tmp"), name="alpha.txt", snippet="Alpha release notes"),
                SearchHit(path=Path("/tmp/beta.txt"), parent_path=Path("/tmp"), name="beta.txt", snippet="Beta launch checklist"),
            ][:limit],
            total=2,
            elapsed_ms=2.0,
        )
