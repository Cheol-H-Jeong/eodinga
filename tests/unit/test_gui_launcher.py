...
def test_launcher_keyboard_flow_supports_arrow_navigation_and_tab_return(qapp) -> None:
    ...
    assert "Alt+P pins the query" in launcher.shortcut_label.text()
...
def test_launcher_context_menu_targets_clicked_result(qapp) -> None:
    def search_fn(query: str, limit: int) -> QueryResult:
        return QueryResult(
            items=[
                SearchHit(path=Path("/tmp/alpha.txt"), parent_path=Path("/tmp"), name="alpha.txt"),
    ...
    second_index = launcher.model.index(1, 0)
    second_center = launcher.result_list.visualRect(second_index).center()
    hit = launcher._context_menu_hit(second_center)

    assert hit is not None
    assert hit.name == "beta.txt"
    assert launcher.result_list.currentIndex().row() == 1
...
def test_launcher_pin_button_and_alt_p_toggle_current_query(qapp) -> None:
    state = LauncherState()
    launcher = LauncherWindow(state=state)
    launcher.show()
...
def test_launcher_pin_button_tracks_pinned_state_for_current_query(qapp) -> None:
    state = LauncherState(pinned_queries=["ext:pdf"])
    launcher = LauncherWindow(state=state)
    launcher.show()
