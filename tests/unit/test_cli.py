...
def test_stats_json_emits_runtime_counters(tmp_path: Path, capsys) -> None:
    ...
    assert payload["commands"]["stats"]["completed"] == 1
    ...
    assert payload["recent_snapshots"][1]["payload"]["format"] == "json"
...
def test_stats_json_structures_failed_command_and_exit_code_counts(tmp_path: Path, capsys, monkeypatch) -> None:
    ...
    assert payload["recent_snapshots"][2]["payload"]["format"] == "json"


def test_stats_json_structures_watcher_failure_and_log_sink_summaries(
    tmp_path: Path,
    capsys,
    ...
    increment_counter("watcher_observer_startup_cleanup_failures", 2)
    increment_counter("watcher_observer_startup_cleanup_failures.stop")
    increment_counter("watcher_observer_startup_cleanup_failures.join")
    ...
    assert payload["watcher_observer_startup_cleanup_failures"] == 2
    assert payload["watcher_startup_rollbacks"] == 1
    assert payload["watcher_failures"] == {
        "observer_cleanup": {"join": 1, "stop": 1},
    ...
    assert payload["recent_snapshots"] == [
        {
            ...
            "payload": {
                "command": "version",
                "elapsed_ms": payload["recent_snapshots"][0]["payload"]["elapsed_ms"],
                "exit_code": 130,
                "reason": "interrupted",
            },
        },
        {
            "name": "command.stats",
            "recorded_at": payload["recent_snapshots"][1]["recorded_at"],
            "payload": {
                "db": str(db_path),
                "documents_indexed": 3,
                "files_indexed": 3,
