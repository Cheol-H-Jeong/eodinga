...
def test_search_json_honors_windows_extended_length_root_filter(cli_runner, tmp_path: Path) -> None:
    db_path = tmp_path / "index.db"
    conn = sqlite3.connect(db_path)
    try:
        apply_schema(conn)
...
def test_search_json_honors_windows_extended_unc_root_filter(cli_runner, tmp_path: Path) -> None:
    db_path = tmp_path / "index.db"
    conn = sqlite3.connect(db_path)
    try:
        apply_schema(conn)
        ...
        for file_id, raw_path, mtime, body_text in (
            ...
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?)
                """,
                (file_id, file_id, "text", mtime, f"sha-unc-{file_id}".encode()),
            )
        conn.commit()
    finally:
        conn.close()
    ...
    assert result.returncode == 0
    payload = json.loads(result.stdout)
    assert [item["path"] for item in payload["results"]] == [r"\\server\share\reports\alpha.txt"]


def test_search_json_honors_windows_extended_length_root_filter_with_lowercase_drive(
    ...
    try:
        ...
        for file_id, raw_path, mtime, body_text in (
            ...
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?)
                """,
                (file_id, file_id, "text", mtime, f"sha-drive-case-{file_id}".encode()),
            )
        conn.commit()
    finally:
        conn.close()
