...
@pytest.mark.parametrize("root_value", ["c:/workspace/reports", r"\\?\C:\workspace\reports"])
def test_search_json_accepts_windows_root_aliases(
    cli_runner, tmp_path: Path, root_value: str
) -> None:
    db_path = tmp_path / "index.db"
    conn = sqlite3.connect(db_path)
    try:
