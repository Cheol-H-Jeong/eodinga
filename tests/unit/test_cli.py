...
def test_search_json_honors_windows_root_filter_with_drive_letter_case_difference(
    ...
    try:
        ...
        for file_id, raw_path, mtime, body_text in (
            (1, r"C:\workspace\reports\alpha.txt", 1_713_528_000, "alpha inside scoped root"),
            (2, r"D:\workspace\reports\alpha.txt", 1_713_527_000, "alpha other drive"),
        ):
            path_obj = PureWindowsPath(raw_path)
            conn.execute(
                """
...
def test_search_json_honors_windows_root_filter_for_long_path_entries(
    cli_runner, tmp_path: Path
) -> None:
    db_path = tmp_path / "index.db"
    conn = sqlite3.connect(db_path)
