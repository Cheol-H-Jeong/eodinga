...
def test_watcher_start_stops_flush_thread_when_schedule_fails(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import eodinga.core.watcher as watcher_module
    ...
    class FakeObserver:
        ...
        def join(self, timeout: float | None = None) -> None:
            raise AssertionError("join should not be reached")
    ...
    assert service._flush_thread is None or not service._flush_thread.is_alive()


def test_watcher_start_stops_started_observer_when_registration_fails(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import eodinga.core.watcher as watcher_module

    stopped: list[Path] = []
    joined: list[tuple[Path, float | None]] = []
    ...
    class FakeObserver:
        ...
        def join(self, timeout: float | None = None) -> None:
            assert self.root is not None
            joined.append((self.root, timeout))
