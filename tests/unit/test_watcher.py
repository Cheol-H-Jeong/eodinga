...
def test_watcher_normalize_root_canonicalizes_extended_unc_prefix_case() -> None:
    normalized = _normalize_root(Path(r"\\?\unc\server\share\reports\\"))

    assert normalized == Path(r"\\?\UNC\server\share\reports")
...
def test_watcher_start_normalizes_equivalent_extended_unc_root_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import eodinga.core.watcher as watcher_module
    ...
    service.start(Path(r"\\?\unc\server\share\reports"))
    service.start(Path(r"\\?\UNC\server\share\reports\\"))
    service.stop()

    assert started == [Path(r"\\?\UNC\server\share\reports")]
