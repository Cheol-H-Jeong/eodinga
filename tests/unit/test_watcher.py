...
def test_watcher_start_cleans_up_when_schedule_fails(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import eodinga.core.watcher as watcher_module
    ...
    class FakeObserver:
        ...
        def start(self) -> None:
            raise AssertionError("observer.start should not run when schedule fails")
...
def test_watcher_start_cleans_up_when_observer_start_fails(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import eodinga.core.watcher as watcher_module
...
def test_watcher_requeued_move_under_backpressure_keeps_source_delete_suppressed(
    tmp_path: Path,
) -> None:
    service = WatchService(queue_maxsize=1)
    first = tmp_path / "first.txt"
    ...
    thread = Thread(target=flush_move, daemon=True)
    thread.start()
    sleep(0.1)

    assert finished is False

    queued_first = service.queue.get_nowait()
    assert queued_first.path == first
