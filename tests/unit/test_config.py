...
def test_load_accepts_pinned_launcher_queries(temp_config_path: Path) -> None:
    temp_config_path.write_text(
        """
[launcher]
pinned_queries = ["ext:pdf", "size:>10M"]
""".strip(),
        encoding="utf-8",
    )
...
def test_load_ignores_unknown_launcher_fields(temp_config_path: Path) -> None:
    temp_config_path.write_text(
        """
[launcher]
frameless = true
...
""".strip(),
    ...
    assert config.launcher.max_results == 42
