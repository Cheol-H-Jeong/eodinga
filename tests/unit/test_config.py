    ...
    config = AppConfig(
        launcher=AppConfig().launcher.model_copy(
            update={
                ...
                "window_width": 800,
                "window_height": 520,
                "pinned_queries": ["report", "content:launch"],
            }
        ),
...
def test_load_accepts_launcher_pinned_queries(temp_config_path: Path) -> None:
...
""".strip(),
    ...
    assert loaded.launcher.pinned_queries == ["report", "content:launch"]
