    ...
    config = AppConfig(
        launcher=AppConfig().launcher.model_copy(
            update={
                ...
                "window_width": 800,
                "window_height": 520,
                "pinned_queries": ["kind:pdf", "date:today"],
            }
        ),
...
def test_load_accepts_persisted_pinned_queries(temp_config_path: Path) -> None:
    temp_config_path.write_text(
        """
[launcher]
pinned_queries = ["kind:pdf", "today"]
""".strip(),
        encoding="utf-8",
    )
    ...
    assert loaded.launcher.pinned_queries == ["kind:pdf", "today"]
