...
def test_load_accepts_pinned_queries_in_launcher_section(temp_config_path: Path) -> None:
    temp_config_path.write_text(
        """
roots = []

[launcher]
pinned_queries = ["alpha", "beta"]
""".strip(),
        encoding="utf-8",
    )
    ...
    assert loaded.launcher.pinned_queries == ["alpha", "beta"]
