...
def test_build_dry_run_returns_zero_and_writes_audit() -> None:
    ...
    assert payload["inno_setup"]["contains_license_file_reference"] is True
    assert payload["inno_setup"]["rendered_contains_license_file_reference"] is True
    assert payload["inno_setup"]["rendered_has_unreplaced_template_tokens"] is False
...
def test_windows_audit_validator_rejects_missing_license_reference() -> None:
    module = _load_build_module()
    payload = module._audit_windows_inputs(__version__, __version__)
    payload["inno_setup"]["contains_license_file_reference"] = False

    errors = module._validate_windows_audit(payload)

    assert "Inno setup no longer references the LICENSE file" in errors


def test_windows_audit_validator_rejects_unreplaced_rendered_tokens() -> None:
    module = _load_build_module()
    payload = module._audit_windows_inputs(__version__, __version__)
    payload["inno_setup"]["rendered_has_unreplaced_template_tokens"] = True

    errors = module._validate_windows_audit(payload)

    assert "Rendered Inno setup still contains unreplaced template tokens" in errors
...
def test_linux_deb_dry_run_stages_recipe() -> None:
    ...
    assert payload["debian_control_template"]["description"] == PROJECT_DESCRIPTION
    assert payload["desktop_entry"]["name"] == "eodinga"
    assert payload["desktop_entry"]["exec"] == "eodinga gui"
    assert payload["desktop_entry"]["icon"] == "eodinga"
    assert payload["desktop_entry"]["categories"] == "Utility;FileTools;"
