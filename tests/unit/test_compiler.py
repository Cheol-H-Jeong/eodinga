...
def test_compile_extended_date_aliases_use_mtime_ranges(query: str) -> None:
    compiled = compile_query(parse(query))
    branch = compiled.branches[0]
...
@pytest.mark.parametrize(
    ...
    [
        ("size:100K..500K", (100 * 1024, 500 * 1024)),
        ("size:100..500K", (100 * 1024, 500 * 1024)),
        ("size:500K..100K", (100 * 1024, 500 * 1024)),
    ],
)
def test_compile_size_range_filter_shape(query: str, expected_params: tuple[int, int]) -> None:
    compiled = compile_query(parse(query))
    branch = compiled.branches[0]
