...
def test_compile_path_regex_with_escaped_separator_preserves_pattern_and_flags() -> None:
    compiled = compile_query(parse(r"path:/tmp\/ims/i"))
    branch = compiled.branches[0]

    assert branch.path_match_sql is None
    assert branch.path_filters == ()
    assert branch.path_regex_terms == (branch.path_regex_terms[0],)
    assert branch.path_regex_terms[0].pattern == r"tmp\/ims"
