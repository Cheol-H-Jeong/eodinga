...
def test_walk_batched_reuses_indexed_at_within_a_batch(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "tree"
    nested = root / "nested"
    ...
    def fake_time() -> float:
        nonlocal time_calls
        time_calls += 1
        return 1_700_000_000 + time_calls
    ...
    assert {record.path for record in records} == {root, nested, nested / "sample.txt"}
