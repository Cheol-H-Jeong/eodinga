...
def test_walk_batched_reuses_discovery_stat_result(
    ...
    child_stat_calls: list[Path] = []
    original_stat_safe = walker_module.stat_safe
    original_scandir_children = walker_module._scandir_children

    def counting_stat(path: Path) -> os.stat_result:
        stat_calls.append(path)
        return original_stat_safe(path)

    def counting_scandir(path: Path) -> list[walker_module._QueuedPath]:
        queued = list(original_scandir_children(path))
        child_stat_calls.extend(item.path for item in queued)
        return queued

    monkeypatch.setattr(walker_module, "stat_safe", counting_stat)
    monkeypatch.setattr(walker_module, "_scandir_children", counting_scandir)
    ...
    assert child_stat_calls.count(nested) == 1
    assert child_stat_calls.count(sample) == 1
...
def test_walk_batched_records_directory_alias_but_skips_reentering_same_inode(
    ...
    def fake_scandir(path: Path) -> list[walker_module._QueuedPath]:
        children = {
            root: [walker_module._QueuedPath(real, fake_stat(real))],
            real: [
                walker_module._QueuedPath(sample, fake_stat(sample)),
                walker_module._QueuedPath(alias, fake_stat(alias)),
            ],
            alias: [
                walker_module._QueuedPath(sample, fake_stat(sample)),
                walker_module._QueuedPath(alias, fake_stat(alias)),
    ...
    monkeypatch.setattr(walker_module, "_scandir_children", fake_scandir)
...
def test_walk_batched_skips_resolved_alias_cycles_even_when_inode_keys_differ(
    ...
    def fake_scandir(path: Path) -> list[walker_module._QueuedPath]:
        children = {
            root: [walker_module._QueuedPath(canonical, fake_stat(canonical))],
            canonical: [
                walker_module._QueuedPath(sample, fake_stat(sample)),
                walker_module._QueuedPath(mirror, fake_stat(mirror)),
            ],
            mirror: [
                walker_module._QueuedPath(sample, fake_stat(sample)),
                walker_module._QueuedPath(mirror, fake_stat(mirror)),
