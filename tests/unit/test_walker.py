...
def test_walk_batched_reuses_scandir_lstat_results(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "tree"
    nested = root / "nested"
    ...
    original_stat_safe = walker_module.stat_safe
    nested_stat = original_stat_safe(nested)
    sample_stat = original_stat_safe(sample)

    def selective_stat(path: Path) -> os.stat_result:
        if path in {nested, sample}:
            raise AssertionError(f"walker restatted discovered child {path}")
        return original_stat_safe(path)

    def fake_scandir(path: Path) -> list[ScandirEntry]:
        children = {
            root: [ScandirEntry(nested, nested_stat)],
            nested: [ScandirEntry(sample, sample_stat)],
        }
        return children.get(path, [])

    monkeypatch.setattr(walker_module, "stat_safe", selective_stat)
